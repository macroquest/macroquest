...
static std::vector<std::shared_ptr<LuaPlugin>> s_pendingPlugins;
...
void LuaPlugin::AddCommand(const std::string& command, sol::function func)
{
...
bool LuaPlugin::RemoveCommand(const std::string& command)
{
	m_commands.erase(command);
	// make sure to call the MQ removecommand here
	return mq::RemoveCommand(command.c_str());
}
...
void LuaPlugin::UnregisterCommands()
{
	...
	{
		// make sure to call the MQ removecommand here
		mq::RemoveCommand(command.c_str());
	}
...
void LuaPlugin::AddDatatype(const std::string& type, sol::table datatype)
{
...
void LuaPlugin::RemoveDatatype(const std::string& type)
{
	m_dataTypes.erase(type);
}
...
void LuaPlugin::AddDatum(const std::string& tlo, sol::function func)
{
	auto [_1, _2, numargs, vararg] = GetArgInfo(m_pluginTable, func);
	...
	else
	{
		...
		m_dataTLOs.insert(tlo);
	}
}

bool LuaPlugin::RemoveDatum(const std::string& tlo)
{
	m_dataTLOs.erase(tlo);

	return mq::RemoveMQ2Data(tlo.c_str());
}
...
{
	const auto& ptr = s_pendingPlugins.emplace_back(std::make_shared<LuaPlugin>(name, version, s));
	ptr->m_pluginTable = sol::state_view(s).create_table_with(
		"__plugin", std::weak_ptr<LuaPlugin>(ptr),
		"addcommand", [](sol::table self, const std::string& command, sol::function func)
			{ if (auto ptr = GetFrom(self)) ptr->AddCommand(command, func); },
		"removecommand", [](sol::table self, const std::string& command)
			{ if (auto ptr = GetFrom(self)) return ptr->RemoveCommand(command); return false; },
		"addtype", [](sol::table self, const std::string& name, sol::table datatype)
			{ if (auto ptr = GetFrom(self)) ptr->AddDatatype(name, datatype); },
		"removetype", [](sol::table self, const std::string& name)
			{ if (auto ptr = GetFrom(self)) ptr->RemoveDatatype(name); },
		"addtlo", [](sol::table self, const std::string& name, sol::function func)
			{ if (auto ptr = GetFrom(self)) ptr->AddDatum(name, func); },
		"removetlo", [](sol::table self, const std::string& name)
			{ if (auto ptr = GetFrom(self)) return ptr->RemoveDatum(name); return false; },
		"name", ptr->m_name,
...
void LuaPlugin::Start(sol::table plugin)
{
	...
	if (!ptr)
	{
		WriteChatStatus("Lua plugin attempted to start, but reference was not found");
	}
	else if (IsRunning(ptr->Name()))
	{
...
{
	return GetFrom(table) != nullptr;
}

void LuaPlugin::ClearPlugins(sol::state_view s)
{
	s_pendingPlugins.erase(std::remove_if(
		s_pendingPlugins.begin(),
		s_pendingPlugins.end(),
		[s](const std::shared_ptr<LuaPlugin>& plugin)
		{
			return !plugin || plugin->m_pluginTable.lua_state() == s.lua_state();
		}), s_pendingPlugins.end());
}
