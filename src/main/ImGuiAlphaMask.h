...
//
// Usage — single layer:
//   mq::imgui::CreateAlphaMaskLayer(draw_list);
...
//   mq::imgui::CreateAlphaMaskLayer(draw_list);   // layer 0
//     mq::imgui::AddImageNineSlice(..., octagonMask);
//   mq::imgui::CreateAlphaMaskLayer(draw_list);   // layer 1
...
// Call any number of times before BeginMaskedDraw.
MQLIB_API void CreateAlphaMaskLayer(ImDrawList* draw_list);
...
//============================================================================
// Framebuffer-alpha ("soft") masking for ImDrawList
//
...
//
// Usage:
//   mq::imgui::CreateAlphaBlendedLayerMask(draw_list, pos_min, pos_max);
//     draw_list->AddImage(...);       // write to fb alpha
//   mq::imgui::BeginAlphaBlendedDraw(draw_list);
//     draw_list->AddRectFilled(...);  // blended by fb alpha coverage
//     draw_list->AddText(...);
//   mq::imgui::EndAlphaBlendedDraw(draw_list);
//
// Notes:
//   - CreateAlphaBlendedLayerMask pushes a clip rect to p_min/p_max and clears
...
//   - Only one soft-mask session should be open at a time.
//============================================================================

// Begins a soft mask session for the given screen region. Pushes a clip rect,
// clears framebuffer alpha to 0 inside the region, and switches to alpha-write
// mode. Draw the mask texture(s) after this call.
MQLIB_API void CreateAlphaBlendedLayerMask(ImDrawList* draw_list, const ImVec2& p_min, const ImVec2& p_max);

// Commits the mask and begins the soft-masked drawing scope. Draw calls issued
...
MQLIB_API void BeginAlphaBlendedDraw(ImDrawList* draw_list);

// Ends the soft-masked drawing scope, restores the default blend state, and
// pops the clip rect pushed by CreateAlphaBlendedLayerMask.
MQLIB_API void EndAlphaBlendedDraw(ImDrawList* draw_list);
