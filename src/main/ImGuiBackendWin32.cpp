...
static void ImGui_ImplWin32_InitMultiViewportSupport(bool platformHasOwnDC);
...
bool ImGui_ImplWin32_InitEx(void* hwnd, bool platformHasOwnDC)
{
	...
	if (io.ConfigFlags & ImGuiConfigFlags_ViewportsEnable)
		ImGui_ImplWin32_InitMultiViewportSupport(platformHasOwnDC);
...
static bool ImGui_ImplWin32_UpdateMouseCursor(ImGuiIO& io, ImGuiMouseCursor imgui_cursor)
{
	...
	// Don't handle mouse cursor if we don't have mouse capture.
	...
	else if (HCURSOR hCursor = static_cast<HCURSOR>(ImGuiManager_GetCursorForImGui(imgui_cursor)))
	{
		::SetCursor(hCursor);
	}
	else
...
static void ImGui_ImplWin32_CreateWindow(ImGuiViewport* viewport)
{
	...
	vd->Hwnd = ::CreateWindowExW(
		...
		vd->HwndParent, nullptr, ghInstance, nullptr);                          // Owner window, Menu, Instance, Param
...
static LRESULT CALLBACK ImGui_ImplWin32_WndProcHandler_PlatformWindow(HWND hWnd, UINT msg, WPARAM wParam, LPARAM lParam)
{
	...
	LRESULT result = 0;
	if (ImGuiManager_HandleWndProcEx(hWnd, msg, wParam, lParam, io))
		result = 1;
...
static void ImGui_ImplWin32_ShutdownMultiViewportSupport()
{
	::UnregisterClassW(L"ImGui Platform", ghInstance);
