...
void ImGui_ImplDX11_RenderDrawData(ImDrawData* draw_data)
{
	...
	// Done without binding the DSV (ClearDepthStencilView does not require it to be bound).
	if (bd->pStencilDSV)
	...
	for (int i = 0; i < 8; ++i) render_state.StencilWriteDSS[i] = bd->pStencilWriteDSS[i];
...
static void ImGui_ImplDX11_DestroyTexture(ImTextureData* tex)
{
	...
	{
		IM_ASSERT(backend_tex->pTextureView == (ID3D11ShaderResourceView*)tex->TexID);
...
float4 main(PS_INPUT input) : SV_Target
{
	...
	// Create the alpha-discard pixel shader (discards texels with alpha < 0.5; used by CreateAlphaMaskLayer)
	{
...
float4 main(PS_INPUT input) : SV_Target
{
	...
	// Create stencil test NOT_EQUAL state — (stencil & 0xFF) != ref, no writes  (used for AlphaMaskOp::Union)
	{
		D3D11_DEPTH_STENCIL_DESC desc = {};
		desc.DepthEnable                  = false;
		desc.StencilEnable                = true;
	...
	// Create no-color blend state (all color channel writes disabled, used during stencil write pass)
	{
		D3D11_BLEND_DESC desc = {};
...
void ImGui_ImplDX11_InvalidateDeviceObjects()
{
	...
	for (ImTextureData* tex : platform.Textures)
	{
		// Ignoring the ref count on the texture because we only have one backend, and ImGui thinks that
		// the refcount on the atlas is somehow a reflection of the number of refcounts on the texture.
		ImGui_ImplDX11_DestroyTexture(tex);
	}
...
struct ImGui_ImplDX11_ViewportData : public ImGuiViewportData
{
...
static void ImGui_ImplDX11_CreateWindow(ImGuiViewport* viewport)
{
	...
	// Assign the device window to our viewport data so we can access list of viewport window handles
