...
//============================================================================
// Build-time state — updated as ImDrawList commands are recorded.
// Draw-list building is single-threaded per context, so plain statics are fine.
//============================================================================
...
//============================================================================
// Callback params — baked at build time, read at render time.
//============================================================================
...
static void BeginMaskedDraw_ImplDX11(const ImDrawList*, const ImDrawCmd* cmd)
{
	...
	// Find 0-based bit index from WriteMask (needed for StencilBitTestEqualDSS lookup).
	int bit_idx = 0;
	for (uint8_t m = params->WriteMask; !(m & 1) && bit_idx < 7; m >>= 1, ++bit_idx) {}
	...
	// Restore the default PS so content draws inside BeginMaskedDraw/EndMaskedDraw
	// are not affected by the alpha-discard PS used in CreateAlphaMaskLayer.
	if (rs->DefaultPS)
...
static void EndMaskedDraw_ImplDX11(const ImDrawList*, const ImDrawCmd*)
{
	...
	// Remove stencil DSV — restore to RTV-only (the imgui backend default).
	ID3D11RenderTargetView* rtv = nullptr;
	ID3D11DepthStencilView* ignoredDSV = nullptr;
	ctx->OMGetRenderTargets(1, &rtv, &ignoredDSV);
	ctx->OMSetRenderTargets(1, &rtv, nullptr);
	if (rtv) rtv->Release();
	if (ignoredDSV) ignoredDSV->Release();

	// Clear the stencil so the next mask session in this frame starts with a clean buffer.
	// (Multiple sessions per frame reuse bit 0; without this the previous shape bleeds into
	//  the next mask's test area.  ClearDepthStencilView does not require the DSV to be bound.)
...
static void CreateAlphaBlendedLayerMask_ImplDX11(const ImDrawList*, const ImDrawCmd*)
{
	auto* rs = static_cast<ImGui_ImplDX11_RenderState*>(ImGui::GetPlatformIO().Renderer_RenderState);
	if (!rs)
		return;
...
//============================================================================
// DX9 callbacks
// The stencil surface is pre-bound by RenderDrawData for the whole frame;
// callbacks only need to update D3DRS_* render states.
//============================================================================
...
static void BeginMaskedDraw_ImplDX9(const ImDrawList*, const ImDrawCmd* cmd)
{
	...
	device->SetRenderState(D3DRS_STENCILWRITEMASK, 0);     // no writes during test
...
static void ClearFbAlpha_ImplDX9(const ImDrawList*, const ImDrawCmd*)
{
	...
	IDirect3DDevice9* device = rs->Device;
	// Force fb alpha to exactly 0: ZERO*src + ZERO*dest = 0.
	device->SetRenderState(D3DRS_ALPHABLENDENABLE, TRUE);
	device->SetRenderState(D3DRS_COLORWRITEENABLE, 0x8);     // alpha only
	device->SetRenderState(D3DRS_SRCBLENDALPHA, D3DBLEND_ZERO);
	device->SetRenderState(D3DRS_DESTBLENDALPHA, D3DBLEND_ZERO);
	device->SetRenderState(D3DRS_STENCILENABLE, FALSE);
}

static void CreateAlphaBlendedLayerMask_ImplDX9(const ImDrawList*, const ImDrawCmd*)
{
	auto* rs = static_cast<ImGui_ImplDX9_RenderState*>(ImGui::GetPlatformIO().Renderer_RenderState);
	if (!rs)
		return;
	...
	IDirect3DDevice9* device = rs->Device;
	// Write only alpha channel: src.a overwrites fb.a (mask coverage).
	device->SetRenderState(D3DRS_ALPHABLENDENABLE, TRUE);
	device->SetRenderState(D3DRS_COLORWRITEENABLE, 0x8);     // alpha only
...
static void BeginAlphaBlendedDraw_ImplDX9(const ImDrawList*, const ImDrawCmd*)
{
	...
	IDirect3DDevice9* device = rs->Device;
	// Blend RGB using framebuffer alpha as coverage.
...
//============================================================================
// Dispatch — resolved once at first use via BackendRendererName.
//============================================================================
...
static ImDrawCallback s_createAlphaBlendedLayerCallback = nullptr;
...
static void EnsureCallbacksResolved()
{
	...
	s_createAlphaBlendedLayerCallback = CreateAlphaBlendedLayerMask_ImplDX11;
	...
	s_createAlphaBlendedLayerCallback = CreateAlphaBlendedLayerMask_ImplDX9;
...
void CreateAlphaBlendedLayerMask(ImDrawList* draw_list, const ImVec2& p_min, const ImVec2& p_max)
{
	EnsureCallbacksResolved();
	...
	draw_list->AddCallback(s_createAlphaBlendedLayerCallback, nullptr, 0);
}
