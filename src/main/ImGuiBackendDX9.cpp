...
static void ImGui_ImplDX9_CreateStencilResources(int w, int h)
{
	...
	bd->pd3dDevice->CreateDepthStencilSurface(
		...
		TRUE,  // Discard — always cleared before use
		&bd->pStencilSurface,
		nullptr);
	bd->StencilWidth  = w;
	bd->StencilHeight = h;
...
void ImGui_ImplDX9_RenderDrawData(ImDrawData* drawData)
{
	...
	// State block does not capture surface bindings, so we save/restore manually.
	...
	// Restore depth-stencil surface binding (not captured by state block)
	if (bd->pStencilSurface)
