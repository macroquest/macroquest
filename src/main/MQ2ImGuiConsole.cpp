...
namespace mq {
	...
	static uint16_t s_mouseHoverTimerSecondsDelay = 12;
...
#pragma region ImGui Console
	...
	public:
		...
		std::chrono::steady_clock::time_point m_pr_StartTime;
		...
		ImGuiWindowFlags GetConsolesFlags()
		{
		...
		void MouseOverUpdate()
		{
			...
			{
				m_pr_StartTime = std::chrono::steady_clock::now();
				s_isMouseInFadeDelay = true;
			}
			else
			{
				if (std::chrono::duration_cast<std::chrono::seconds>(std::chrono::steady_clock::now() - m_pr_StartTime ) < std::chrono::seconds(s_mouseHoverTimerSecondsDelay))
				{
					s_isMouseInFadeDelay = true;
				}
				else
				{
					s_isMouseInFadeDelay = false;
				}
			}
		}
		...
		{
			ImGuiWindowFlags windowFlags = GetConsolesFlags();

			ImGui::SetNextWindowSize(ImVec2(640, 240), ImGuiCond_FirstUseEver);
			ImGui::PushStyleVar(ImGuiStyleVar_WindowPadding, ImVec2(1, 0));
			ImGui::SetNextWindowBgAlpha(s_isMouseInFadeDelay ? s_alphaConsoleNormal * .01f : s_alphaConsoleFade * .01f);
			...
			m_zepEditor->mq::imgui::ImGuiZepEditor::SetFont(Zep::ZepTextType::Text, ImGui::GetFont(), s_consoleFontSize);
