...
const char* GetPipeName(uint32_t index)
{
	...
	if (config != s_postOfficeConfigs.end() && config->second.PipeName)
		return config->second.PipeName->c_str();

	return mq::MQ_PIPE_SERVER_PATH;
}

ActorIdentification GetCurrentID(uint32_t index)
{
	...
	{
		if (config->second.AccountOverride)
			client.account = *config->second.AccountOverride;

		if (config->second.ServerOverride)
			client.server = *config->second.ServerOverride;

		if (config->second.CharacterOverride)
			client.character = *config->second.CharacterOverride;
	}
...
public:
	...
	void RouteMessage(MessagePtr message) override
	{
		...
		*message->mutable_return_address()->mutable_client() = GetCurrentID(m_index).GetProto().client();
...
private:
	...
	uint32_t m_index; // store the post office index for self-lookups (useful for testing)
