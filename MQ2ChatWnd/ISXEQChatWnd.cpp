...
{
	sprintf(szChatXMLSection, "%s.%s", EQADDR_SERVERNAME, ((PSPAWNINFO)pLocalPlayer)->Name);
	...
	pISInterface->GetSetting(XMLFileName, szChatXMLSection, "Title", Buffer, sizeof(Buffer));
	if (Buffer[0] == '\0') {
		strcpy(Buffer, "ISXEQ");
