...
public:
	...
	// Actor Access
	virtual void SendToActor(postoffice::Dropbox* dropbox, const proto::routing::Address& address, uint16_t messageId, const std::string& data, MQPlugin* owner) = 0;
	virtual void ReplyToActor(postoffice::Dropbox* dropbox, ProtoMessagePtr&& message, uint16_t messageId, const std::string& data, uint8_t status, MQPlugin* owner) = 0;
