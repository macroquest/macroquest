...
function(parse_vcpkg_mq_files MQ_FILES OUT_PACKAGES OUT_DISCARDED)
    set(ALL_PACKAGES "")
    set(ALL_DISCARDED "")
    ...
    list(LENGTH ALL_PACKAGES ALL_PKG_COUNT)

    foreach (x ${ALL_PACKAGES})
        message(STATUS ${x})
    endforeach ()

    merge_package_entries("${ALL_PACKAGES}" MERGED_PACKAGES)
    set(${OUT_PACKAGES} "${MERGED_PACKAGES}" PARENT_SCOPE)
    set(${OUT_DISCARDED} "${ALL_DISCARDED}" PARENT_SCOPE)
endfunction()
        ...
        foreach(PATH ${VCPKG_OVERLAY_PORTS_PATH})
            ...
            if(IS_DIRECTORY ${PORT_DIR})
                if((OVERLAY_FORCE_REBUILD AND PATH STREQUAL "${CMAKE_SOURCE_DIR}/plugins") OR NOT EXISTS "${PORT_DIR}/vcpkg.json")
                    ...
                    # next, create the package name for the template (we will always be producing a file)
                    set(VCPKG_NAME ${PORT_LOWER})

                    # finally, create the file from the template. add the port only if legacy files exist to process
                    if(MQ_FILES)
                        message(STATUS "  Processing vcpkg dependencies: ${PORT_LOWER} -> ${PORT_DIR}")
                        list(LENGTH MQ_FILES MQ_FILE_COUNT)

                        # Parse all vcpkg_mq files found
                        parse_vcpkg_mq_files("${MQ_FILES}" VCPKG_DEPENDENCIES DISCARDED_TRIPLETS)
                        if(VCPKG_DEPENDENCIES)
                            # Remove duplicates from discarded list (discarded is necessarily a subset of packages)
                            if(DISCARDED_TRIPLETS)
                                list(REMOVE_DUPLICATES DISCARDED_TRIPLETS)
                            endif()

                            # Format as JSON for template
                            format_packages_as_json("${VCPKG_DEPENDENCIES}" VCPKG_DEPENDENCIES)
                            format_list_as_json("${DISCARDED_TRIPLETS}" DISCARDED_TRIPLETS)
                    ...
                    elseif(EXISTS "${PORT_DIR}/vcpkg.json")
                        ...
                        message(STATUS "  Reusing vcpkg dependencies: ${PORT_LOWER} -> ${PORT_DIR}")
                        list(APPEND ALL_PORTS ${PORT_LOWER})
                        list(APPEND PORT_DIRS ${PORT_DIR})
                    elseif(OVERLAY_FORCE_REBUILD)
                        # only display this message if a force was specified
                        message(STATUS "  Found empty vcpkg dependencies: ${PORT_LOWER} -> ${PORT_DIR}")
                    endif()
                elseif(EXISTS "${PORT_DIR}/vcpkg.json")
                    list(APPEND ALL_PORTS ${PORT_LOWER})
                    list(APPEND PORT_DIRS ${PORT_DIR})
