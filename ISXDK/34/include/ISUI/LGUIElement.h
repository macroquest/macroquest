...
public:
	...
	LGUI_API virtual void OnNotify(LGUIElement *pElement, unsigned int Notification, unsigned int Value) {if (pParent) pParent->OnNotify(pElement,Notification,Value);};
	...
	inline bool GetAlwaysOnTop() {return bAlwaysOnTop;}
	inline void SetAlwaysOnTop(bool NewValue) {bAlwaysOnTop=NewValue;}
